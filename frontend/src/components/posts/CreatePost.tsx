...
const FileChip = ({
  ...
  return (
    <div className={`flex items-center justify-between gap-3 rounded-2xl border px-3 py-2 ${wrapperClass}`}>
      ...
      <button type="button" onClick={onRemove} className="grid h-8 w-8 place-items-center rounded-full text-red-500
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3">
        <button type="button" onClick={handleAvatarClick} className="shrink-0 rounded-full cursor-pointer transition
          hover:opacity-80"
        >
          <Avatar
            src={avatarSrc}
        ...
        <div className="rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2">
          <div className="flex items-center gap-2">
            <AutoResizeTextarea value={title}
              onChange={(event) =>
                setTitle(event.target.value)
              }
              placeholder="Bạn đang nghĩ gì?" className="max-h-52 min-h-[40px] w-full bg-transparent text-sm text-slate-700
              outline-none placeholder:text-slate-400"
            />

            <button type="button" onClick={openFilePicker} className="grid h-9 w-9 shrink-0 place-items-center rounded-full
            text-slate-500 transition hover:bg-slate-200 hover:text-slate-800"
            >
      ...
      {file && fileKind && (
        <div className="mt-3 grid grid-cols-[auto_minmax(0,1fr)_auto] gap-3">
          <div />
          <FileChip name={fileName} kind={fileKind} onRemove={handleRemoveFile} />
          <div />
        </div>
      )}
