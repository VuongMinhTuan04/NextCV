      ...
      <div className="hidden flex-1 items-center justify-center gap-4 sm:flex">
        <nav>
          <ul className="flex items-center gap-6">
            <li>
              <button onClick={onHomeClick} className={`flex cursor-pointer items-center gap-2 rounded-full px-3 py-2
        ...
        <div className="relative w-72">
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Tìm người dùng..."
            className="w-full rounded-full border border-slate-200 bg-white py-2 pl-4 pr-10 text-sm outline-none
          ...
          {results.length > 0 && search.trim() !== "" && (
            <div className="absolute left-0 top-12 z-50 w-full rounded-2xl border border-slate-200 bg-white shadow-lg">
              <ul className="max-h-72 overflow-y-auto py-2">
                {results.map((user) => (
                  <li key={user.id} onClick={() => handleUserClick(user.id)} className="flex cursor-pointer items-center
                    gap-3 px-4 py-2 hover:bg-slate-100"
                  >
